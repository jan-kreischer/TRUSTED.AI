...
# from helper_functions import get_case_inputs
...
def calc_explainability_score(model, X_train, X_test, config):
    ...
    output = dict(
        Algorithm_Class     = score_Algorithm_Class(model, clf_type_score),
        Correlated_Features = score_Correlated_Features(X_train, X_test),
        Model_Size          = score_Model_Size(model, config),
        Feature_Relevance   = score_Feature_Relevance(model, scale_factor, distri_threshold)
                 )
    return output